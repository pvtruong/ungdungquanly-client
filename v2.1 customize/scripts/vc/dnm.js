var dnmModule = new baseVoucher('dnm','dnm',["so_ct","ma_hd","dien_giai","ma_kh","ten_kh"],'Đề nghị mua hàng',{
     onLoading:function($scope,$options){
       ...
       $scope.printBarcode = function(){
        ...
        $scope.createContract = function(denghimua){
            var _contract = JSON.parse(JSON.stringify(denghimua));
            _contract.id_dnm = _contract._id;
            delete _contract._id;
            delete _contract.so_ct;
            _contract.ngay_ct = new Date();

            purchase_contractModule.quickadd($options.$uibModal,function(ct){
              $options.service.get(id_app,denghimua._id).then(function(res){
                _.extend(denghimua,res.data);
              })
            },_contract);
        }
    ...
    onInput:function($scope,$options){
        ...
        var addSPMOI = function(item,callback){
            ...
            if(dnmModule.defaultValues4Detail){
                for(var key in dnmModule.defaultValues4Detail){
                    nrow[key] =dnmModule.defaultValues4Detail[key];
                }
        ...
        $scope.tinhtienhang = function(nrow){
            ...
            dnmModule.tinhCkvt($options.$http,$scope.data,nrow);
        }
...
dnmModule.defaultValues ={

}
dnmModule.defaultValues4Detail = {
	so_luong:0,
	ty_le_ck:0,
	gia_nt:0,gia:0,
	tien_nt:0,tien:0,
...
dnmModule.defaultCondition4Search = {tu_ngay:new Date(),den_ngay:new Date(),so_ct:'',dien_giai:'',ma_kh:''};
dnmModule.prepareCondition4Search = function($scope,vcondition){
	return {
...
dnmModule.watchMaster = function(scope,options){
