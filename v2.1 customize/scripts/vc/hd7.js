var hd7Module = new baseVoucher('hd7', 'hd7', [], 'Trả lại hoặc đổi hàng', {
  onInput: function(scope, options) {
    scope.selectItem = function(data, item, callback) {
      if(!item.ma_vt) return;
      if (!callback)
        callback = function() {};
...
hd7Module.defaultValues = {
  vatvaos: []
}
hd7Module.defaultValues4Detail = {
  sl_nhap: 0,
  gia_von_nt: 0,
  gia_von: 0,
  gia_ban_nt: 0,
...
hd7Module.defaultCondition4Search = {
  tu_ngay: new Date(),
  den_ngay: new Date(),
  so_ct: '',
  dien_giai: '',
...
hd7Module.prepareCondition4Search = function($scope, vcondition) {
  return {
    so_ct: {
...
hd7Module.watchDetail = function(scope) {
  ...
  scope.$watch('dt_current.gia_von_nt', function(newData) {
    if (newData != undefined && scope.status.isOpened) {
      scope.dt_current.gia_von = STP.round(scope.dt_current.gia_von_nt, scope.f_tien);
      scope.dt_current.tien_nhap_nt = STP.round(scope.dt_current.sl_nhap * scope.dt_current.gia_von_nt, scope.round);;
      scope.dt_current.tien_nhap = STP.round(scope.dt_current.sl_nhap * scope.dt_current.gia_von_nt, scope.f_tien);
    }
  });
  ...
  scope.getInvoice = function() {
    var modalInstance = scope.$uibModal.open({
      templateUrl: "templates/vouchers/hd7/templates/form-select-invoice.html",
      controller: [
        '$scope',
        '$uibModalInstance',
        'parentScope',
        ...
        function($scope, $uibModalInstance, parentScope, $filter, $rootScope) {
          ...
          $scope.loadInvoice = function() {
            $scope.masters = [];
            $scope.details = [];
            var url = server_url_report + "/api/" + id_app + "/getinvoice2return_bl?t=1";
...
hd7Module.watchMaster = function(scope) {
