var sale_contractModule = new baseVoucher('sale_contract','sale_contract',["ma_hd","so_ct","dien_giai","ma_kh","ten_kh"],'Hợp đồng bán hàng',{
    ...
    onLoading:function($scope,$options){
        ...
        $scope.createHD2 = function(contract){
            var f_tien = $options.$rootScope.app_info.options.f_tien;
            ...
            hd.ngay_ct = new Date();
            if(hd.t_thue){
                hd.ngay_hd = hd.ngay_ct;
            }

            hd.details = _.filter(hd.details,function(d){
              return d.so_luong>d.sl_da_xuat;
            })

            hd.details.forEach(function(d){
                d.sl_xuat = $options.$rootScope.app_info.options.sl_xuat_sl_hop_dong?d.so_luong-(d.sl_da_xuat||0):0;
                d.gia_ban = d.gia;
                d.gia_ban_nt = d.gia_nt;
                d.tien_nt = STP.round(d.sl_xuat*d.gia_ban_nt,f_tien_nt);
                d.tien = STP.round(d.sl_xuat*d.gia_ban_nt,f_tien);
            })



            hd2Module.quickadd($options.$uibModal,function(ct){
    ...
    onInput:function($scope,$options){
        ...
        $scope.select = function(item,up){
            ...
            if(f.length>0){
                ...
                if(sale_contractModule.defaultValues4Detail){
                    _.extend($scope.dt_current,sale_contractModule.defaultValues4Detail);
                }
...
sale_contractModule.defaultValues ={
	t_thue_nt:0,t_thue:0,thue_suat:0,t_ck_nt:0,tk_ck:0
}
sale_contractModule.defaultValues4Detail = {
	so_luong:0,
	ty_le_ck:0,
	gia_nt:0,gia:0,
	tien_nt:0,tien:0,
...
sale_contractModule.defaultCondition4Search = {tu_ngay:new Date(),den_ngay:new Date(),so_ct:'',dien_giai:'',ma_kh:''};
sale_contractModule.prepareCondition4Search = function($scope,vcondition){
	return {
...
sale_contractModule.watchDetail = function(scope){
...
sale_contractModule.watchMaster = function(scope){
